...
{
    public class SalesChartBackgroundService : BackgroundService
    {
        ...
        public SalesChartBackgroundService(
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                if (groups.Count() > 0)
                {
                    ...
                    {
                        var data = await chartService.GenerateSalesData(groupId);

                        await _hubContext.Clients.Group(groupId).ReceiveSalesChartData(data);
                    }
                }
